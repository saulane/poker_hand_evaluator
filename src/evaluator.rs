...
pub fn eval_5cards(cards: &Vec<Card>) -> u16 {
    let mut q: u32 = (cards[0].bit_value
        | cards[1].bit_value
        | cards[2].bit_value
        | cards[3].bit_value
        | cards[4].bit_value)
    ...
    q = (cards[0].bit_value & 0xff)
        * (cards[1].bit_value & 0xff)
        * (cards[2].bit_value & 0xff)
        * (cards[3].bit_value & 0xff)
        * (cards[4].bit_value & 0xff);
...
pub fn eval_hand(board: &Vec<Card>, hand: &Vec<Card>) -> u16 {
    match board.len() {
        3 => return eval_5cards(&vec![board[0], board[1], board[2], hand[0], hand[1]]),
        5 => return eval_7hand(board, hand),
        _ => return 0,
    }
}
